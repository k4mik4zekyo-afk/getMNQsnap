...
def plot_anchored_volume_profile(df, start_time=None, end_time=None, bins=40, 
                                  value_area_pct=0.70, filename="mnq05_anchored_vp.html"):
    """
    Plot an interactive anchored volume profile with POC, VAH, and VAL.
    ...
    if start_time is None or end_time is None:
        ...
        # Default: 6:30 AM PST previous day to 5:30 AM PST current day
        if start_time is None:
            start_time = now.normalize() - pd.Timedelta(days=1) + pd.Timedelta(hours=6, minutes=30)
        if end_time is None:
    ...
    for i, (center, volume) in enumerate(zip(centers, hist)):
        # Determine if in value area
        in_value_area = lower_index <= i <= upper_index
        
        # Create color gradient (darker = more volume)
        volume_pct = volume / max_volume
        if in_value_area:
            # Blue gradient for value area
            r, g, b = 70, 130, 180  # Steel blue base
            intensity = 0.3 + (volume_pct * 0.7)  # 30% to 100% intensity
            color = f'rgba({int(r*intensity)}, {int(g*intensity)}, {int(b*intensity)}, 0.8)'
        else:
            # Coral gradient for outside value area
            r, g, b = 240, 128, 128  # Light coral base
            intensity = 0.3 + (volume_pct * 0.7)
            color = f'rgba({int(r*intensity)}, {int(g*intensity)}, {int(b*intensity)}, 0.6)'
        ...
        volume_pct_total = (volume / total_volume) * 100
        hover_text = (
            f"<b>Price Range:</b> {price_low:.2f} - {price_high:.2f}<br>"
            f"<b>Center:</b> {center:.2f}<br>"
            f"<b>Volume:</b> {volume:,.0f}<br>"
            f"<b>% of Total:</b> {volume_pct_total:.2f}%<br>"
            f"<b>Area:</b> {'Value Area' if in_value_area else 'Outside VA'}"
        )
    ...
    title=dict(
        text=(
            f"<b>Anchored Volume Profile (MNQ 1m)</b><br>"
            f"<sup>{start_time.strftime('%Y-%m-%d %H:%M')} to {end_time.strftime('%Y-%m-%d %H:%M')} PST | "
            f"Bar Width: {bar_width:.2f} points</sup>"
        ),
        x=0.5,
        xanchor='center'
    ),
    ...
    showlegend=False,
    height=800,
    width=1600,  # Changed from 1000
    margin=dict(r=800)  # Added this line
)
    
    # Add annotations box with statistics
    fig.add_annotation(
        text=(
            f"<b>Statistics</b><br>"
            f"Total Volume: {total_volume:,.0f}<br>"
            f"Value Area: {value_area_pct*100:.0f}%<br>"
            f"VA Range: {vah_price - val_price:.2f} pts<br>"
            f"POC Volume: {poc_volume:,.0f}"
        ),
    ...
    print(f"Saved interactive volume profile to {output_path}")
    ...
    print(f"Bar Width: {bar_width:.2f} points")
...
def export_mnq01_volume_profile(mnq05, start_time=None, end_time=None, bins=40, value_area_pct=0.70):
    """
    Export MNQ 1-minute interactive volume profile.
    """
    return plot_anchored_volume_profile(
        ...
        filename="mnq01_anchored_vp.html",  # Changed to .html
    )
...
if __name__ == "__main__":
    ...
    mnq01 = get_MNQ(time_interval="1m",
                    start_time=start,
                    end_time=recent)
    ...
    export_mnq01_volume_profile(mnq01, value_area_pct=0.68)  # 68% instead of 70%